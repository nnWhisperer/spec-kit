...
description: Run the per-feature orchestrator end-to-end (specify → clarify → plan
  → tasks → optional analyze → implement).
compatibility: Requires spec-kit project structure with .specify/ directory
metadata:
  author: github-spec-kit
  source: subagent-flow:commands/speckit.subagent-flow.run.md
---
...
2. When it returns, run `bash .specify/scripts/bash/check-prerequisites.sh --json --paths-only` and parse the JSON to confirm the spec file exists on disk.
